...
def rotate_images(images, rot90_scalars=(0, 1, 2, 3)):
    images_rotated = [images, images.transpose(2, 3), images.flip(2),
                      images.transpose(2, 3).flip(3)]
    results = torch.cat([images_rotated[i] for i in rot90_scalars], dim=0)
    return results


def merge_with_rotation_data(real, fake, real_labels, fake_labels, num_rot_examples):
    real_to_rot, fake_to_rot = (real[-num_rot_examples:], fake[-num_rot_examples:])
    real_rotated = rotate_images(real_to_rot, rot90_scalars=(1, 2, 3))
    fake_rotated = rotate_images(fake_to_rot, rot90_scalars=(1, 2, 3))
    all_features = torch.cat([real, real_rotated, fake, fake_rotated], dim=0)

    real_rotated_labels = real_labels[-num_rot_examples:].repeat(3, 1)
    fake_rotated_labels = fake_labels[-num_rot_examples:].repeat(3, 1)
    all_labels = torch.cat([real_labels, real_rotated_labels,
                            fake_labels, fake_rotated_labels], dim=0)
