...
def hinge_losses(d_real_logits, d_fake_logits, d_real=None, d_fake=None):
    d_loss_real = torch.mean(F.relu(1.0 - d_real_logits))
    d_loss_fake = torch.mean(F.relu(1.0 + d_fake_logits))
